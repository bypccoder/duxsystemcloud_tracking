...
class TaskController extends Controller
{
    ...
    {
        $this->middleware(['role:Admin|Backoffice', 'permission:admin.tasks.index|admin.tasks.create|admin.tasks.edit|admin.tasks.destroy']);
    }
    ...
    function getResult2(Request $request)
    {

        $result1_id = $request->input('result1_id');

        $result2s = Result2Back::where('result1_backs_id', $result1_id)->get();

        $result2Options = [];

        foreach ($result2s as $result2) {
            $result2Options[$result2->id] = $result2->result;
        }
